...
def parse_args():
    parser = argparse.ArgumentParser(description="Choose a classifier for validation.")
    parser.add_argument(
        "--classifier_type",
        type=int,
        choices=[1, 2],
        default=2,  # Default to Dog Breed
        help="1 for Cat-Dog classifier, 2 for Dog Breed classifier"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
...
def main():
    ...
    args = parse_args()
    classifier_type = "catdog" if args.classifier_type == 1 else "dogbreed"
