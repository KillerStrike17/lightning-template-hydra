...
@task_wrapper
def infer(model, image_tensor, classifier_type):
    model.eval()
    with torch.no_grad():
...
@task_wrapper
def main(args):
    if args.classifier_type == "catdog":
    ...
    with get_rich_progress() as progress:
        ...
        for image_file in image_files:
            if image_file.suffix.lower() in ['.jpg', '.jpeg', '.png']:
                ...
                predicted_label, confidence = infer(model, img_tensor.to(model.device), args.classifier_type)
...
if __name__ == "__main__":
    ...
    # Set default input folder to 'sample'
    ...
    parser.add_argument("--classifier_type", type=int, choices=[1, 2], default=2, help="1 for Cat-Dog classifier, 2 for Dog Breed classifier")
    ...
    args.classifier_type = "catdog" if args.classifier_type == 1 else "dogbreed"
